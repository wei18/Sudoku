...
**This skill shipped without a TDD baseline test** (see `superpowers:writing-skills` Iron Law). The loophole list below is best-effort, captured retroactively from one incident (PR #263 — production AdMob App ID leaked into Info.plist comment) and one Architect review (`meetings/2026-06-03_admob-secret-injection-architect-review.md`). Treat the rationalizations table as incomplete; revise on next leak incident.
...
When one repo ships multiple app schemes (e.g. Sudoku + Minesweeper), XCC sets `$CI_PRODUCT` and `$CI_XCODE_SCHEME` per workflow. Case-switch on the scheme to pick the right env-var prefix:

```bash
case "${CI_XCODE_SCHEME:-${CI_PRODUCT:-}}" in
  Sudoku)
    APP_ID="${SUDOKU_ADMOB_APP_ID:?missing SUDOKU_ADMOB_APP_ID}"
    BANNER_UNIT_ID="${SUDOKU_ADMOB_BANNER_UNIT_ID:?missing SUDOKU_ADMOB_BANNER_UNIT_ID}"
    ;;
  Minesweeper)
    APP_ID="${MINESWEEPER_ADMOB_APP_ID:?missing MINESWEEPER_ADMOB_APP_ID}"
    BANNER_UNIT_ID="${MINESWEEPER_ADMOB_BANNER_UNIT_ID:?missing MINESWEEPER_ADMOB_BANNER_UNIT_ID}"
    ;;
  *)
...
1. **Production IDs in code comments, docstrings, PR descriptions, commit messages, or `Info.plist <!-- -->` blocks.** Memory `scrub-prod-secrets-from-comments` documents the 2026-06-03 incident (PR #263). Even when the value field uses a sandbox stand-in, the surrounding prose leaks production via git history. **Including the literal ID anywhere in tracked text — even prefixed by TODO / FIXME / "will-replace" — IS the leak.** Reference the memory file by name; never paste the value.
...
|---|---|
| "But this is just a TODO comment, not the actual value" | Including the literal ID anywhere in tracked text IS the leak (PR #263). Reference memory by name. |
...
- Memory `scrub-prod-secrets-from-comments` — the incident that triggered this skill
- Memory `admob-production-ids` / `minesweeper-admob-ids` / `asc-api-credentials` — the actual real values held outside repo

## Migration playbook

**Disclosure**: PR1 (2026-06-03, AdMob migration) compressed the Architect's recommended 7-stage plan into a single PR. The sequence below reflects PR1's actual landing pattern, NOT the gold-standard staging. Architect's full 7-PR plan is at `meetings/2026-06-03_admob-secret-injection-architect-review.md` §"Migration plan" for reference.

| PR | Scope | Notes |
|---|---|---|
| 1 | Scaffolding + Info.plist substitution + Live.swift Bundle.main + smoke tests + ci_post_clone.sh extension + secrets/.env.example (all in one) | PR1's actual shape. Architect would split into 3 PRs. |
