...
Fact grounding cited inline as `[VF]` / `[ARCH]` from `docs/marketing/BRIEF.md`.
...
**8/**
Product footprint to match: no third-party analytics, no accounts beyond iCloud, no tracking. [VF: Privacy]

Sudoku is free with a removable banner + one-time Remove Ads — removable, not ad-free. [VF: Monetization]

Public since the first commit. Solo-built.
...
A calm Sudoku for iPhone and Mac. No tracking, no accounts beyond your own iCloud. [VF: Privacy]

Saves and records sync through iCloud; daily leaderboards via Game Center. [VF: iCloud sync, Game Center]

Free, with an optional one-tap Remove Ads. [VF: Monetization]
...
## Notes for Leader
- App Store / repo links omitted; Sudoku in submission prep, Minesweeper not submitted. [VF: Status] `[UNVERIFIED — Leader confirm]` public store/TestFlight URL and canonical repo URL before posting either variant.
- Hashtag counts kept ≤3/post.
- No userbase / ranking / award numbers (Do-Not-Claim). Sudoku framed as removable banner, never "ad-free".
