...
§ Catalog      apple-dev-skills (20) · collaboration-skills (10) · aggregated external (3, by-ref · credited)
...
2. The README Catalog lists exactly: every first-party skill across both plugins **plus**
   the three aggregated external plugins (set-equality, both directions).
3. Hardcoded counts (README group headers `(20)`/`(10)`/`(3)`, each `plugin.json`
...
- Both first-party plugins install and load their skills:
  ...
  `collaboration-skills@apple-dev-skills` (10); the three externals still install (verified
