...
//
// #1021 Phase B (Leader ruling 3): `DailyStripView` (the old, now-deleted
// week-strip card) used to own `isTappable(_:)`; the rule — a completed,
// REVIEWABLE, PAST day is tappable — now lives in `TodayMapper.headerModel`'s
// `tappablePipIndices` (built from the exact same `DailyStripDay.isReviewable`
// + `isToday` check). Pinned here at the mapper level instead of a view.

@Suite("TodayMapper — pip interactivity (#826)")
struct TodayMapperPipInteractivityTests {
    ...
    /// Wraps `day` as the sole entry in a 1-day `DailyStripSnapshot` and asks
    /// whether the mapper marks index 0 tappable — same gate a real 7-day
    /// window applies per-index.
    private func isTappable(_ day: DailyStripDay) -> Bool {
        let snapshot = DailyStripSnapshot(days: [day], streak: nil)
        let model = TodayMapper.headerModel(
            weekStrip: snapshot,
            allCompletedDays: [],
    ...
    /// is the single gate for all three.
