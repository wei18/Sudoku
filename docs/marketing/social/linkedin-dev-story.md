...
Fact grounding cited inline as `[VF: <row>]` (Verified Facts table) or `[ARCH]` (Architecture facts) from `docs/marketing/BRIEF.md`. Privacy framing = Path B (no absolute "no tracking").
...
On the product side I kept the footprint deliberately small: no first-party analytics SDK building a profile of the player, and no account beyond the player's own iCloud. [VF: Privacy] To be precise rather than slogan-y: Sudoku integrates AdMob, so it ships a removable banner and ads may use the iOS ad identifier for relevance behind the ATT prompt — decline and ads still serve, or remove ads entirely with a one-time purchase. The `PrivacyInfo.xcprivacy` declares that honestly instead of claiming "no tracking." [VF: Privacy, Monetization] Saves and records sync through the player's own iCloud. [VF: iCloud sync]

The whole repo has been public since the first commit — architecture docs, review cycles, and the decision logs are all readable. If you want to see how it's actually wired rather than how I describe it, that's the more honest artifact: https://github.com/wei18/Sudoku

Solo-built. [ARCH] Sudoku (v2.5) is on the App Store: https://apps.apple.com/app/id6771248206 — happy to talk through any of the boundaries above.
...
Privacy posture to match: no first-party analytics SDK profiling the player, no account beyond their own iCloud. [VF: Privacy] Free with a removable banner + one-time Remove Ads — removable, not ad-free; tailored ads are opt-in via ATT, or remove them entirely. [VF: Monetization]

Public since the first commit, so the wiring is readable, not just describable: https://github.com/wei18/Sudoku — Sudoku on the App Store: https://apps.apple.com/app/id6771248206. Solo-built.
...
## Notes for Leader
- Sudoku is on the App Store (v2.5); Minesweeper (v1.0) not yet submitted. [VF: Status] MS App Store page (live once approved): https://apps.apple.com/app/id6775733519 — omit from the post until you want to surface MS.
- Repo URL: https://github.com/wei18/Sudoku (filled).
- No userbase / ranking / award numbers used (Do-Not-Claim). Privacy stated via Path B framing (no absolute "no tracking").
