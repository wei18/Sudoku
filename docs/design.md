...
**Status: DRAFT — 含 prerequisite checklist 須於 plan.md 第一階段驗證後升 FINAL**（見 §How.4.9）。
...
- [ ] **SplitMix64 跨架構 bit-identical 驗證** — **Unconfirmed**。Plan.md Phase 0 寫一條最小 test：固定 seed `0x9E3779B97F4A7C15`、跑前 10 次 `next()`、與手算 expected 序列比對；在 iPhone 16 simulator (arm64) 與 Mac arm64 各跑一次，輸出必須完全一致。
- [ ] **Performance baseline** — **Unconfirmed**。Plan.md Phase 0 prototype generator + uniqueness validator，在 iPhone 15 simulator (Apple Silicon dev machine) 上量 Hard 題 generation：median < 300 ms、p95 < 500 ms。失敗則本節 §How.4.7 上限需上調或演算法需重看。
- [ ] **App Store policy spot-check** — **Unconfirmed**。本機產生的隨機 puzzle 上 leaderboard 屬主流模式（NYT mini Sudoku、Apple News+ Crossword 等均如此），App Review Guidelines 2026 版未針對此模式設限（主要規範集中在 1.2 user-generated content / AI-generated content 揭露）。Plan.md 落地時搜尋並截圖近 12 個月 App Store Review notes / Apple Developer forum 提及「randomly generated leaderboard content」之案例，存 `meetings/` 備查。
...
| `foundations.md §3` snapshot 由主畫面起步 | §How.5.8 明列 21 張 |
...
```
        ┌──────────────────────────────┐
        │  Snapshot (UI)               │  SudokuUITests，§How.5.8 起手 21 張
        ├──────────────────────────────┤
...
| Coverage target | 行覆蓋不適用；以 **§How.5.8 的 21 張 baseline 全部存在且通過** 為 must-cover。新 View 入 v1 前 PR 必須補對應 snapshot |
