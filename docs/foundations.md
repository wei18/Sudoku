...
- **`docs/` + `meetings/` 遷移至 GitHub Wiki**（2026-05-26）：當前 spec / RFC / meeting log 全住 `Sudoku-spec` repo 樹下；隨 docs 累積，wiki 比 file tree 更適合做 narrative-heavy 文件的瀏覽（auto sidebar / cross-link / web 搜尋 / 對外公開但不動 code repo 結構）。兩條落地選項可獨立或組合：(1)（optional）`sudoku` 主 repo 加 submodule 指向 `wei18/Sudoku.wiki.git`，讓 spec/meetings 跟 code 在同一 clone 樹下，本機 grep + agent 一次 load 仍方便；(2) 整理 `docs/` `meetings/` 結構讓每份文件可一對一搬到 wiki page（檔名 → page title、相對連結 → wiki link、frontmatter 拿掉、圖片改走 wiki assets、`_Sidebar.md` / `_Footer.md` 補上）。觸發點：(i) 第二人 collab 需要 wiki-style discoverability；(ii) `meetings/` 累積 > 50 份單檔 navigate 變慢；(iii) 想對外公開部分 spec 但不想動 repo 結構。代價：wiki history 跟 PR review flow 不同（無 squash / 無 required check / 無 code review subagent）、cross-repo link 維護、CI 上的 lychee link-check 需重設目標、`[[feedback-...]]` memory link 慣例需要對應 wiki link 規則。
