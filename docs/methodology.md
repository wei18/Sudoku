...
- **Running implementation notes log**（2026-05-20）：在 `meetings/` 新增一種 agent log — subagent 執行任務時持續更新 `meetings/{date}_{topic}.impl-notes.md`（or `.html`）。內容四欄：(1) 設計決定—規格含糊處 subagent 做了什麼選擇；(2) 偏離—有意偏離規範的部分與原因；(3) 折衷—考慮過的替代與為何選現方案；(4) 未決—希望 Leader / User 確認的事項。差別於現有 phase meeting log（事後總結）— impl-notes 是 *during work* 持續更新，捕捉中途 micro-decisions。需更新 `ai-collaboration-mode` SKILL.md 把這條納入 subagent dispatch contract。
