...
//     rail — see the sibling DigitPadView+Mac.swift.
//
...
struct DigitPadView: View {
    ...
    // `internal` (not `private`) — the Mac arrangement in the sibling
    // DigitPadView+Mac.swift reads it across files within the same module,
    // matching BoardView's own internal-for-file-split convention.
