...
- **Acknowledgements 頁（App Store 第三方授權清單）**：由 LicensePlist 在 `ci_post_clone.sh` 跑 `mise exec ubi:mono0926/LicensePlist -- license-plist` 從 SwiftPM dep graph 自動產 `App/Resources/Settings.bundle/`（iOS 標準位置，顯示於 Settings.app → Sudoku → Acknowledgements）。Source of truth: 倉庫根 `license_plist.yml`；產出檔 `.gitignore`'d，每次 build 重生。
...
- **補齊 repo skills/context 缺口**（2026-05-24 reflection）：今天連續兩次 parallel dispatch race 暴露幾個 skill/context gap，可考慮陸續補：(a) `subagent-conflict-detection` skill — 派發前自動 grep 目標 file set 跟 in-flight subagent 比對；(b) `pr-diff-verification` skill — commit 前強制 `git show --stat HEAD` 比對 commit message 跟實際變更，避免 PR #126 那種「commit log 寫了但 code 沒進」事故；(c) `asc-ops-handoff` skill — TestFlight / ASC API / Apple Developer 流程的 step-by-step；(d) `monetization-sdk-integration` skill — foundations §9 break-glass 的 hands-on guide；(e) memory 改成 `feedback/` `project/` `reference/` 子目錄而非 flat。觸發點：每條獨立評估，當第二次踩同個坑時動工。
