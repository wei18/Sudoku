...
//
// Mounted beside `submitWinIfWon()` (same file-split rationale: keeps
// the main class body under the 400-line lint ceiling) but as its OWN latch
// (`didEvaluateAchievements`), because achievement evaluation must only run
// on the LIVE `reveal()` win transition (it increments a non-idempotent
// cumulative win tally), whereas `submitWinIfWon()` also re-runs
// (idempotently) from `refresh()` — sharing one latch would either skip
// achievements on a live win reached via `refresh()`'s snapshot pull, or
// re-inflate the win tally on a later refresh.
//
...
// the win moment — same posture as `submitWinIfWon()`.
...
extension MinesweeperGameViewModel {
    ...
    /// `reveal()` (NOT `refresh()` — unlike `submitWinIfWon()` this is
    ...
    /// `submitWinIfWon()` so this win's GC/personal-best writes have already
    /// landed before the full-spectrum / streak facts are gathered below.
