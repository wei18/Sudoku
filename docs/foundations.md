...
- Swift 官方宣布支援 Android 後，純 Swift module（典型如 `SudokuEngine`）有機會以 Swift Package 形式輸出給 Android Studio 使用；§2 模組切法應預留這個可能（2026-05-15）。與 `docs/v1/design.md §不在 v1 範圍 §平台與輸入` 的「Android via Swift SDK」條目對偶。
