...
BRIEF Verified Facts; nothing claims a feature the BRIEF forbids. Tone: calm, understated,
no superlatives, no emoji (brand contract). Each caption is a short overlay headline + an
optional one-line subhead, sized for legibility on a phone screenshot.
...
- **Frame:** Stylised App Privacy "nutrition label" / a calm graphic of an iCloud lock, the
  app UI behind it.
- **Overlay headline:** `Private by design.`
- **Subhead:** `No tracking. Saves stay in your iCloud.`
- **Backs:** Verified — no third-party tracking SDK; saves in your iCloud Private Database.
- **Guard:** Do NOT say "no ads / ad-free" — Sudoku has a removable banner.
...
### Frame 4 — Privacy
- **Frame:** Calm iCloud-lock / privacy-label graphic over the board.
- **Overlay headline:** `Private by design.`
- **Subhead:** `No tracking. No backend of our own.`
- **Backs:** Verified — no third-party tracking; listing "no CRM, no backend of our own."
- **Guard:** Do NOT claim "syncs your games" (only monetization state syncs for MS); do NOT
  say "ad-free" (banner planned/removable per listing — see flag).
...
- `[UNVERIFIED — Leader confirm]` Minesweeper monetization state on screenshots: BRIEF says MS
  ...
  Remove Ads frame should appear for MS v1 at all.
