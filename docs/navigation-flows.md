...
| N20 | GC signed-out — Completion entry point | #698 deleted the dead leaderboard-fetch state machine (Completion VMs no longer compute `.unauthenticated` at all — the popup never had a leaderboard zone since v2.6). Any future GC signed-out feedback on Completion is a new-feature decision, not this gap; the surviving GC entry points (Home card / Settings row) are #685's scope | — |
