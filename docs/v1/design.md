...
> **§How.5 AS-BUILT UPDATE (2026-07-05)**：以下內容為 v1 凍結敘述，保留不改；實際
> 現況已因 SDD-003/#664/#669/#660/#677 產生分歧，更新如下 — board 呈現自 SDD-003
> 起在 iOS 為 modal `fullScreenCover`（macOS 仍維持 push）；completion 自 #664/#669
> 起在所有平台統一為 board 內的 `CompletionOverlayScaffold` overlay（pushed 的
> `.completion` route 現僅服務 daily re-view，Sudoku #379 / MS #386）；pause + leave
> 自 #660 起合併為單一選單（`PauseOverlayView`）；timer 自 #677 起移入 board 自身的
> header/status row。權威 flow spec 見 `docs/navigation-flows.md` +
