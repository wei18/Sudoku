...
_自 2026-05-26 起，新 backlog 項目改建 GitHub issue（label `backlog` + `draft` + 1-2 個 topic label：`ci` / `modules` / `testing` / `tooling` / `documentation` / `devx`）。下表為遷移對照；歷史完整文字見 git history（`git show <pre-migration-sha>:docs/foundations.md`）._

**新增方式**：`gh issue create --label "backlog,draft,<topic>"`，title 為簡短的條目主旨，body 沿用 「Date / 描述 / Trigger / 代價」 結構。
...
| 2026-05-23 | 抽 Telemetry / GameCenterClient / Persistence 成獨立 SPM package | 🟡 PARTIAL — Telemetry done PR [#161](https://github.com/wei18/Sudoku/pull/161); GameCenterClient + Persistence = Stage 3 (in-session task) |
