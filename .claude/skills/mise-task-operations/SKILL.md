...
| `mise run scan:store_frame_drift [--base <ref>\|--list]` | ASC frame drift proxy gate: fails when a snapshot baseline that FEEDS a `Slot()` in `scripts/build-ascspec-screenshots.py` changed without a matching `docs/app-store/screenshots-ascspec/` update (#995/#998, required CI job in `lint.yml`); mapping is parsed fresh from the script every run, no hardcoded slot list | safe (read-only gate) | — (#1001) |
