...
| N4 | Sudoku Daily `.exhausted` (generator defect) | `Color.clear` backdrop + floating `.alert` "Couldn't generate today's puzzle" / "Try a different difficulty, or come back tomorrow." — **"Practice"** swaps the last path entry `.daily` → `.practice` (→ `SUD-PRACTICE-HUB`, which has the picker); **"Cancel"** pops back to HOME (#686, fixes the prior dead-`{}`-closure + no-recovery trap) | `SudokuUI/Daily/DailyHubView.swift` `.alert(...)`; `DailyHubViewModel.swift` `tryPracticeInstead()` / `dismissExhausted()` |
