...
- `ci_scripts/ci_post_clone.sh` — runs on every XCC clone: installs mise tools,
  ...
  defence. → [[xcode-cloud-single-track-ci]], [[apple-public-repo-security]]
- `ci_scripts/ci_pre_xcodebuild.sh` — pre-build hook. → [[xcode-cloud-single-track-ci]]
...
- [[xcode-cloud-single-track-ci]] — the CI workflows these tasks mirror or temporarily substitute.
